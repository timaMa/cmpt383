...
execute 'go get github.com/SebastiaanKlippert/go-wkhtmltopdf' do
	cwd project_home 
	user username
	environment 'HOME' => user_home
	creates user_home + '/go/src/github.com/SebastiaanKlippert/go-wkhtmltopdf/README.md'
end

execute 'gem install ffi' do
	creates "#{ruby_gems}/ffi/ffiFile"
end

execute 'gem install commonmarker' do
end
...
execute "sudo apt-get -y install libfontconfig1 libxrender1" do
	cwd project_home 
	user username
	environment 'HOME' => user_home
end

execute "go build -o pdf_converter.so -buildmode=c-shared pdf_converter.go" do
	cwd project_home 
	user username
	environment 'HOME' => user_home
end

execute 'pip3 install rst2pdf' do
 creates "#{python3_packages}/rst2pdf/__init__.py"
end

execute 'pip3 install PyPDF2' do
 creates "#{python3_packages}/PyPDF2/__init__.py"
end
